...
int run_four_op_vectors(Top* top, const std::string& path, int rec_w, const std::string& label) {
    ...
    while (std::getline(fin, line)) {
        ...
        std::string mode_s, cmp_s, in1_s, in2_s, in3_s, in4_s, out_s, exc_s;
        if (!(iss >> mode_s >> cmp_s >> in1_s >> in2_s >> in3_s >> in4_s >> out_s >> exc_s)) {
        ...
        const u128 in4 = parse_hex_u128(in4_s);
        ...
        set_wide(top->io_in_bits_in4, in4);
        top->io_in_valid = 1;
        step_clock(top);
        top->io_in_valid = 0;
        ...
        if (!valid || !recfn_matches_expected(actual_out, expected_out, rec_w, cmp_mode) || actual_exc != expected_exc) {
            print_four_op_context(label, rm, in1, in2, in3, in4, actual_out, expected_out, actual_exc, expected_exc);
            return 1;
        }
        ++tests;
    }
